    ...
    if (row.length > 0) {
      ...
      if (subElement.length > 0) {
        ...
        const [results] = await db.execute(
          `SELECT id, title, title_slug, summary, image_big, image_mid, created_at FROM posts WHERE category_id IN (${placeholder}) ORDER BY created_at DESC LIMIT 10 OFFSET ${offset}`,
          subId,
        );

        const [totalCountResult] = await db.execute<RowDataPacket[]>(
          `SELECT COUNT(*) AS totalCount FROM posts WHERE category_id IN (${placeholder})`,
          subId,
        );
