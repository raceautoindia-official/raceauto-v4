...
export async function GET() {
  try {
    ...
    const [joinedRow] = await db.execute<RowDataPacket[]>(
      `SELECT posts.id, posts.image_small, posts.title, posts.pageviews, posts.title_slug, posts.created_at, posts.user_id, posts.is_featured, posts.featured_order, posts.featured_order, posts.is_featured, posts.is_breaking, posts.is_recommended, posts.category_id, categories.name_slug AS name_slug, categories.parent_id AS parent_id, categories.color AS color, categories.name AS sub_category, users.username AS username 
        FROM posts
