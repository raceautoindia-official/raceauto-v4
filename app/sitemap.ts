...
const getISOString = (date: string) => {
  return isValidDate(date) ? new Date(date).toISOString() : new Date().toISOString();
}
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [tags]: any = await db.execute(`SELECT DISTINCT tag, tag_slug FROM tags`);
  ...
  const [market]: any = await db.execute(`SELECT title, title_slug FROM post_market`);
  ...
  const sitemap: MetadataRoute.Sitemap = [
    ...
    ...pages.map((page: any) => ({
      url: `${process.env.BACKEND_URL}/page/${page.slug}`,
      changeFrequency: 'weekly',
      priority: 0.6,
    })),
    ...market.map((page: any) => ({
      url: `${process.env.BACKEND_URL}/market/${page.title_slug}`,
      changeFrequency: 'weekly',
      priority: 0.5,
    })),
    ...mainCategory.map((category: any) => ({
      url: `${process.env.BACKEND_URL}/category/${category.name_slug}`,
      changeFrequency: 'monthly',
      priority: 0.7,
    })),
    ...articles.map((article: any) => ({
      url: `${process.env.BACKEND_URL}/post/${escapeXML(article.title_slug)}`,
      ...
      images: article.image_mid
        ? [
            {
              url: `${process.env.BACKEND_URL}${article.image_mid}`,
    ...
    // ...tags.map((tag: any) => ({
    //   url: `${process.env.BACKEND_URL}/tag/${tag.tag_slug}`,
    //   changeFrequency: 'monthly',
    //   priority: 0.5,
    // })),
  ];
