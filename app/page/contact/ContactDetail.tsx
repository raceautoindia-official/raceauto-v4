...
const ContactDetail = () => {
  ...
  const contactData = async () => {
    try {
      const res = await axios.get(`${process.env.BACKEND_URL}api/contact`);
      ...
      <div className="row g-0 p-5">
        <div className="col-sm-6">
          {data.map((item: contactType, i) => (
            <div className="" key={i}>
              <h2>Our Office</h2>
              <p><IoLocationSharp className="me-3" size={25}/>{item.contact_address}</p>
              <p><FaPhoneAlt className="me-3" size={25}/>{item.contact_phone}</p>
              <p><MdEmail className="me-3" size={25}/>{item.contact_email}</p>
            </div>
          ))}
        </div>
        <div className="col-sm-6">
