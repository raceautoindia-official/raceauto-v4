...
const EventCard = async ({ item }: { item: eventType }) => {
  ...
  return (
    ...
    <a href={item.referenceLink} target="_blank" rel="noreferrer" style={{color:"inherit"}}>
        <div className="row">
            ...
            <div className="col-md-8">
                <h6 className="event__date" style={{ fontWeight: 700, }}>{item.event_date}</h6>
                <h5 className="event__title" style={{ fontWeight: 900, }}>{item.title}</h5>
                <p className="event__summary">{item.summary}</p>
                <p className="event__location">Location: {item.location}
                </p>
            </div>
        </div>
    </a>
