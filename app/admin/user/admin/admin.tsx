                    ...
                    <Table striped bordered hover>
                        ...
                        <tbody>
                            {users.map((user:any) => (
                                <tr key={user.id}>
                                    <td>{user.id}</td>
                                    <td>{user.avatar ? <Image src={`${process.env.BACKEND_URL}/${user.avatar}`} alt="avatar" /> : <FaUserCircle size={35} />}</td>
                                    <td>{user.email}</td>
                                    <td>{user.role}</td>
                                    {/* <td>{user.created_date}</td> */}
                                    <td>
                                    <Link href={`/admin/user/${user.id}`}><Button variant="warning">Edit</Button></Link>{' '}
                                        <Button variant="danger" onClick={() => handleShow(user.id)}>Delete</Button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
