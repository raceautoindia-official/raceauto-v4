...
const RoleForm = () => {
    ...
    const roleApi = async () => {
        try {
            const res = await axios.get(
                `${process.env.BACKEND_URL}api/admin/role/${id}`,
    ...
    const handleSubmit = async () => {
        try {
            ...
            await axios.put(
                `${process.env.BACKEND_URL}api/admin/role/${id}`, formData,
    ...
    return (
        <div className="col-12">   
            <Link href='/admin/admin-access'><Button variant="secondary" className="mt-3 ms-2" >Back</Button></Link>
                ...
                <Form>
                    {Object.keys(permissions).map((permission:any) => (
                        <Form.Check
                            key={permission}
                            type="checkbox"
                            label={permission.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                            name={permission}
                            className='mt-2'
